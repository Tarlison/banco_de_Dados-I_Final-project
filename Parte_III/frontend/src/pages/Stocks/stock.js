...
class Tabela extends Component{
  ...
  hendleDelete(data){
    ...
    Axios.delete(`http://localhost:3333/produtos/${DelDataJson.id}`,{
      headers: {
    ...
    for (let i = 0 ; i<jsonObj.length;i++){
      componentsObjects.push(
        <tr key={jsonObj[i].id}>
          ...
          <td>{jsonObj[i].descricao}</td>
          <td>{jsonObj[i].preco}</td>
          <td>{jsonObj[i].quantidade}</td>
          <td>{jsonObj[i].fornecedor}</td>
          <td>
            <button 
              id={jsonObj[i].id} 
